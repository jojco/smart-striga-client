<template>
    <div class="flex flex-col gap-8 grow">
        <UCard>
            ...
            <div v-else>
                <div class="flex flex-col gap-2">
                    <UMeter color="primary" :value="SCD30?.co2Concentration" label="CO2 Concentration" :max="3000" :min="0">
                        <template #indicator>
                            <div class="font-black text-right">
                                {{ SCD30?.co2Concentration.toFixed(2) }} ppm
                            </div>
                        </template>
                    </UMeter>
                    ...
                    <UMeter color="primary" :value="SCD30?.relativeHumidity" label="Relative Humidity" :max="100" :min="0">
                        <template #indicator>
                            <div class="font-black text-right">
                                {{ SCD30?.relativeHumidity.toFixed(2) }} %
                            </div>
                        </template>
                    </UMeter>
            ...
            <template #footer>
                <div class="flex justify-end">
                    <UButton size="lg" icon="i-ph-arrows-clockwise-duotone" @click="refreshSCD30()">Refresh SCD30</UButton>
                </div>
            </template>
        </UCard>
        ...
        <UCard>
            ...
            <template #footer>
                <div class="flex justify-end">
                    <UButton size="lg" icon="i-ph-arrows-clockwise-duotone" @click="refreshW1()">Refresh W1</UButton>
                </div>
            </template>
        </UCard>
...
const { pending: pendingSCD30, data: SCD30, refresh: refreshSCD30 } = await useLazyFetch<SCD30Response>(`${config.public.api}/scd30`)
const { pending: pendingW1, data: W1, refresh: refreshW1 } = await useLazyFetch<W1SensorResponseList>(`${config.public.api}/w1`)

const { pending: pendingGPIO, data: gpio, execute } = await useLazyFetch<W1SensorResponseList>(`${config.public.api}/gpi`, { method: 'POST', params: {pin: 29, on: 'on'} })
